...
def get_random_message():
    messages = [
"Ultrathink to fix all errors based on the Next.js 15 and TypeScript documentation, and write super clean code as a senior developer. After each fix, change the [ ] to [x] in errors.md, but do not do this after fixing multiple errors at once. If an error has already been fixed and requires no further changes, simply mark it as [x] and proceed to the next error. Stricktly I order you NOT create new files. I have all the files, but they might be in different folders or have different names. fix only up to 10 errors at a time."
]
